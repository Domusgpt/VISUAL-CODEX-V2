...
class PolytopBackgroundVisualizer {
        ...
        class VisualCodexGallery {
            ...
            updateTagChipStates() {
                ...
                const statusEl = document.getElementById('controlDeckStatus');
                if (statusEl) {
                    statusEl.textContent = this.activeTagFilter ? `Tag • ${this.activeTagFilter}` : this.baseControlDeckStatus;
                }
            }
            ...
            updateTagFocusList() {
                ...
                const entries = this.tagIndex.get(this.activeTagFilter) || [];
                if (!entries.length) {
                    focusEl.classList.add('empty');
                    focusEl.innerHTML = `No entries mapped to ${this.activeTagFilter}.`;
                    return;
                }
                ...
                const listItems = entries.map((entry) => {
                    ...
                    return `
                        <div class="${itemClass}">
                            ...
                            <div class="control-deck-focus-meta">Section ${entry.sectionIndex + 1} • ${sectionName}</div>
                        </div>
                    `;
                }).join('');

                focusEl.innerHTML = `
                    <div class="control-deck-focus-title">${this.activeTagFilter} — ${entries.length} items</div>
            ...
            updateNavigationTagStates() {
                const hasFilter = Boolean(this.activeTagFilter);
                ...
                sections.forEach((section) => {
                    ...
                    const match = hasFilter && tags.has(this.activeTagFilter);

                    section.classList.toggle('tag-match', match);
                    section.classList.toggle('tag-muted', hasFilter && !match);

                    if (hasFilter) {
                        section.setAttribute('data-tag-match', match ? 'true' : 'false');
                    } else {
            ...
            updateWaferHighlighting() {
                ...
                for (let i = 0; i < 3; i++) {
                    ...
                    wafer.classList.remove('tag-match', 'tag-muted');
                    ...
                    if (content && Array.isArray(content.tags) && content.tags.includes(this.activeTagFilter)) {
