...
*   **Key Features & Effects:**
    ...
    *   **Focus Review Mode:** A chip toggle labelled “focus review” applies a `focus-mode` body class that tames holographic glows, deepens the control shells, and updates the state indicator so copy reviews can happen against a calmer canvas without touching other presets.
...
*   **Focus Review Toggle:** Clicking the focus chip toggles the `focus-mode` class, flips the live chip label between “ON”/“OFF,” and announces the change through the polite live region alongside the status indicator entry.
