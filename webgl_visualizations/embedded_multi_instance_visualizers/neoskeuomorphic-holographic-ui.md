...
.depth-layer.background { transform: translateZ(-160px) scale(1.1); }
.neomorphic-card {
...
.neomorphic-card:hover {
    transform: translateZ(24px) rotateX(-4deg) rotateY(3deg);
    box-shadow:
...
*   **Ripple Feedback:** Clicks retrigger the `.interaction-ripple` animation by removing and re-adding the `active` class.

---

#### Demo Reference

*   The standalone demo (`demos/neoskeuomorphic-holographic-ui-demo.html`) condenses the GEMINI asset into a gallery-safe HTML file, maintaining layered depth, grid overlays, chaos pulses, and ripple feedback without external dependencies.
