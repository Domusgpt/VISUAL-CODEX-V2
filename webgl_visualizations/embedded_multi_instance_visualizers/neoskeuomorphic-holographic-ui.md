...
*   **Key Features & Effects:**
    ...
    *   **Snapshot Memory & Contrast Boost:** A dedicated snapshot panel captures the current layout, depth multiplier, glow strength, theme preset, and pointer orbit preference, saves it to `localStorage`, and exposes restore/copy-to-clipboard actions. Inputs adopt consistent `:focus-visible` rings, and a `(prefers-contrast: more)` subscription enhances card/ribbon borders for assistive readability.
...
*   **Snapshot Workflow:** Save/restore buttons serialize the deck state, reapply it (including pointer orbit and glow settings), and announce actions via the live region while keeping restore/copy disabled until a snapshot exists.
