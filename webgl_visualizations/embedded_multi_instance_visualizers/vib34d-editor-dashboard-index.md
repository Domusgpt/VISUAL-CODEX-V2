...
*   **Timed Application:** `setTimeout` advances through the 25 states at 120 ms intervals. For every tick the script updates CSS custom properties and synchronizes the visible metrics list, ensuring both visuals and textual metadata evolve together.
...
*   The demo (`demos/vib34d-editor-dashboard-index-demo.html`) reproduces the holographic cards, gradient grids, and 25-iteration cascade in a standalone HTML file for gallery embedding.
*   Cards remain responsive down to 320 px widths thanks to `auto-fit` grid templates and clamped padding, keeping the GEMINI-era layout accessible for mobile previews.
