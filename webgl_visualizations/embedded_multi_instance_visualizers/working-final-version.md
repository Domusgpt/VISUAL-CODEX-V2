...
*   The gallery demo (`demos/working-final-version-demo.html`) encapsulates the ribbon, visualizer spine, telemetry matrix, story deck, calibration dock, auto loop, palette dropdown, and resettable status LED so the consolidated build can be previewed without the original Windows environment while respecting `(prefers-reduced-motion)` constraints.
