...
*   Added snapshot persistence, clipboard export, and `(prefers-contrast: more)` handling across all demos so reviewers can resume exact states, copy JSON payloads, and inspect high-contrast variants directly from the gallery build.
*   Launched the Codex Pt. 3 suite with four reimagined demos in `demos/codex-pt3/`, keeping prior work intact while delivering wholly new interactions for gallery explorers.
